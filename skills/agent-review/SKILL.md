...
4. Run `agent-review.mjs open <file>` in the active turn and retain its running process/session. Open watches continuously by default; add `--no-open` only when the browser is already open. Give the user the localhost URL if needed.
5. Do not finish the agent turn while the review remains open.
6. For each compact JSON line emitted by the open process, handle every item, then wait on that same process again. Never launch a replacement one-shot poll between items.
...
10. When the target is chat, answer in the browser with `agent-review.mjs reply <file> <concise-reply>`, then wait on the same open process.
11. Stop only when the open process emits `{"status":"ended"}`. Use `watch` only to attach without opening a browser, and `poll` only for a deliberate one-shot diagnostic.

The open process emits compact JSON lines like:
